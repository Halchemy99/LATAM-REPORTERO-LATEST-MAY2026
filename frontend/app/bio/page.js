...
const SOCIAL_LINKS = [
  { label: 'TikTok', handle: '@latamreportero', href: 'https://tiktok.com/@latamreportero' },
  { label: 'Instagram', handle: '@latamreportero', href: 'https://instagram.com/latamreportero' },
  { label: 'YouTube', handle: '@latamreportero', href: 'https://youtube.com/@latamreportero' },
];
  ...
  if (status === 'success') {
    return (
      <div className="flex items-center gap-3 p-4 bg-[#6111ff]/15 border border-[#6111ff]/30">
        ...
        <div>
          <p className="text-white text-sm font-medium">{t('newsletter.successTitle')}</p>
          <p className="text-white/50 text-xs mt-0.5">{t('newsletter.successDesc')}</p>
        </div>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="flex gap-2">
      ...
      >
        {status === 'loading' ? (
          ...
          <>{t('pricing.free')} <ArrowRight className="h-3.5 w-3.5" /></>
        )}
      </button>
    </form>
  );
      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        <Link href="/" className="flex items-center justify-between p-4 bg-white/5 border border-white/10 hover:border-white/25 transition-colors group">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white text-sm font-medium">{t('bio.readStories')}</span>
          </div>
        ...
        <Link href="/investigations" className="flex items-center justify-between p-4 bg-white/5 border border-white/10 hover:border-white/25 transition-colors group">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white text-sm font-medium">{t('bio.deepDives')}</span>
          </div>
          <ArrowRight className="h-4 w-4 text-white/30 group-hover:text-white/70 transition-colors" />
        </Link>
        ...
        <Link href="/community" className="flex items-center justify-between p-4 bg-white/5 border border-white/10 hover:border-white/25 transition-colors group">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white text-sm font-medium">{t('bio.joinCommunity')}</span>
          </div>
          <ArrowRight className="h-4 w-4 text-white/30 group-hover:text-white/70 transition-colors" />
        </Link>
